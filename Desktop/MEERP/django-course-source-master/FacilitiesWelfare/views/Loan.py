...
class UserLoanReg(LoginRequiredMixin,DefineRegisterLoan,CreateView):
    model = RegisterLoan
    success_url = reverse_lazy('FacilitiesWelfare:Loan-List-User')
    template_name = "FacilitiesWelfare/Loan/RegisterLoan-create-update.html"

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        IdLoan = self.kwargs.get('IdLoan')        
        context['LoanDetail'] = Loan.objects.filter(Id = IdLoan)
        
        return context

class UserLoanReg2(LoginRequiredMixin,DefineRegisterLoan,CreateView):
    model = RegisterLoan
