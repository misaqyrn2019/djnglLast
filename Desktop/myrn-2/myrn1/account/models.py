...
class City(models.Model):
    ...
    def __str__(self):
        return "استان " + str(self.IdProvince) + " شهر " + self.Name
...
class User(AbstractUser):
    ...
    is_HR = models.BooleanField(default=False, verbose_name="مدیر منابع انسانی")
    HR_user = models.DateTimeField(default=timezone.now, verbose_name="کاربر منابع انسانی تا")
    ...
    def is_HR_user(self):
        if self.HR > timezone.now():
            return True
        else:
            return False
    is_HR.boolean = True
    is_HR.short_description = "وضعیت منابع انسانی تا"